...
const GEOMETRY_IS_RECT = 1u;
const GEOMETRY_HAS_ALPHA = 2u;

struct BlendInstance {
    @location(0) geometry_xy: u32,
    @location(1) geometry_extent: u32,
    @location(2) geometry_alpha_col_idx: u32,
    @location(3) geometry_flags: u32,
    @location(4) parent_texture_origin: u32,
    @location(5) target_texture_size: u32,
    @location(6) child_texture_origin: u32,
    @location(7) child_rect_origin: u32,
    @location(8) child_rect_size: u32,
    @location(9) blend_rect_origin: u32,
    @location(10) blend_config: u32,
}
...
struct VertexOutput {
    ...
    @location(6) @interpolate(flat) geometry_flags: u32,
...
fn vs_main(
    ...
    let geometry_height = select(
        4u,
        geometry_extent.y,
        (instance.geometry_flags & GEOMETRY_IS_RECT) != 0u,
    );
    let geometry_size = vec2<f32>(f32(geometry_extent.x), f32(geometry_height));
    let parent_xy = vec2<f32>(geometry_xy) + vec2<f32>(x, y) * geometry_size;
    let scene_xy = vec2<f32>(blend_rect_origin)
        + parent_xy
        - vec2<f32>(parent_texture_origin);
    ...
    out.tex_coord = vec2<f32>(
        ...
        y * f32(geometry_height),
    );
    out.geometry_flags = instance.geometry_flags;
...
fn fs_main(
    ...
    if (geometry_flags & GEOMETRY_HAS_ALPHA) != 0u {
