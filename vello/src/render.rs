...
pub(crate) fn record_fine_segment_draft(
    ...
    recording.dispatch(
        fine_area_load_draft,
        fine_wg,
        [config_buf, session.segments_buf, session.ptcl_buf, session.info_bin_data_buf, session.blend_spill_buf, ResourceProxy::Image(out_image), session.gradient_image, session.image_atlas, session.effect_params_buf, ResourceProxy::Image(base), ResourceProxy::Image(draft), ResourceProxy::Image(region)],
    );
    recording.free_resource(config_buf);
    recording
}
...
/// [`record_fine_segment_draft`] but binding 10 is the chained gather's PRIMARY input `input` (the
/// previous link's materialised surface) instead of a blur draft. `base` still holds the original
/// backdrop. Used by a frosted lens's blur-H (taps `input` = warp), scatter (`input` = blurred), and
/// tail shade (`input` = scattered, `base` = backdrop for maskmix).
#[cfg(feature = "wgpu")]
pub(crate) fn record_fine_segment_input(
    session: &mut PhasedSession,
    shaders: &FullShaders,
    seg_lo: u32,
    seg_target: u32,
    ...
    let fine_area_load_input = shaders
        ...
        .expect("a chained gather needs the fine_area_load_input shader");
    let wg_counts = &session.cpu_config.workgroup_counts;
    let mut recording = Recording::default();
    ...
    recording.dispatch(
        fine_area_load_input,
        fine_wg,
        [config_buf, session.segments_buf, session.ptcl_buf, session.info_bin_data_buf, session.blend_spill_buf, ResourceProxy::Image(out_image), session.gradient_image, session.image_atlas, session.effect_params_buf, ResourceProxy::Image(base), ResourceProxy::Image(input), ResourceProxy::Image(region)],
    );
    recording.free_resource(config_buf);
    recording
}
...
pub(crate) fn record_fine_segment_rwu(
    ...
    seg_target: u32,
    snap: ImageProxy,
    slot10: Option<(bool, bool, ImageProxy)>,
    region: ImageProxy,
    out_image: ImageProxy,
) -> Recording {
    let mut recording = Recording::default();
    ...
    match slot10 {
        ...
        Some((is_draft, packed, extra)) => {
            let shader = match (is_draft, packed) {
                (true, false) => {
                ...
                (true, true) => shaders
                    ...
                    .expect("packed staging needs fine_area_rwu_draft_pk"),
                (false, true) => shaders
                    .fine_area_rwu_input_pk
                    .expect("packed staging needs fine_area_rwu_input_pk"),
            };
            recording.dispatch(
                shader,
                fine_wg,
...
#[cfg(feature = "wgpu")]
pub(crate) fn record_fine_segment_stg(
    session: &mut PhasedSession,
    shaders: &FullShaders,
    seg_lo: u32,
    seg_target: u32,
    ...
    let shader = shaders.fine_area_stg.expect("packed staging needs fine_area_stg");
    let mut recording = Recording::default();
    let (config_buf, fine_wg) = seg_config(session, &mut recording, seg_lo, seg_target);
    recording.dispatch(
        shader,
...
/// A MATERIALIZE window over the packed accumulator backdrop writing the packed staging store
/// (`fine_area_stg_load*`): `base` is the accumulator bound read-only, `sdf` optionally a sampled
/// field texture at slot 10, value reads ride the staging store itself.
#[cfg(feature = "wgpu")]
pub(crate) fn record_fine_segment_stg_load(
    session: &mut PhasedSession,
    shaders: &FullShaders,
    seg_lo: u32,
    seg_target: u32,
    ...
    match sdf {
        None => {
            let shader = shaders.fine_area_stg_load.expect("packed staging needs fine_area_stg_load");
            recording.dispatch(
                shader,
                fine_wg,
                [config_buf, session.segments_buf, session.ptcl_buf, session.info_bin_data_buf, session.blend_spill_buf, ResourceProxy::Image(out_image), session.gradient_image, session.image_atlas, session.effect_params_buf, ResourceProxy::Image(base)],
        ...
        Some(sdf) => {
            let shader =
                shaders.fine_area_stg_load_sdf.expect("packed staging needs fine_area_stg_load_sdf");
            recording.dispatch(
                shader,
                fine_wg,
...
/// A CHAIN materialize window (`fine_area_stg_chain*`): taps and value ride the packed staging
/// store (the read-write output); `base` is the packed accumulator, read-only, serving
/// backdrop-edge taps and orig.
#[cfg(feature = "wgpu")]
pub(crate) fn record_fine_segment_stg_chain(
    session: &mut PhasedSession,
    shaders: &FullShaders,
    seg_lo: u32,
    seg_target: u32,
    ...
    let shader = if is_draft {
        shaders.fine_area_stg_chain_draft.expect("packed staging needs fine_area_stg_chain_draft")
    } else {
        shaders.fine_area_stg_chain.expect("packed staging needs fine_area_stg_chain")
    };
    let mut recording = Recording::default();
    let (config_buf, fine_wg) = seg_config(session, &mut recording, seg_lo, seg_target);
    recording.dispatch(
