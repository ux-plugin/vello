...
pub struct FullShaders {
    ...
    /// `output`, `base_in` / `input_in` / `region_atlas` as r32uint sampled slots at 9/10/11, and
...
pub(crate) fn full_shaders(
    ...
    let fine_resources_packed = [
        Uniform,
        BufReadOnly,
        BufReadOnly,
        BufReadOnly,
        Buffer,
        ImageArrayReadWrite(ImageFormat::R32Uint),
        ImageRead(ImageFormat::Rgba8),
        ImageRead(ImageFormat::Rgba8),
        BufReadOnly,
    ...
    let fine_packed = aa_support
        .area
        .then(|| add_shader!(fine_packed, fine_resources_packed, CpuShaderType::Missing));
    let snap_copy = aa_support.area.then(|| {
        add_shader!(
            snap_copy,
